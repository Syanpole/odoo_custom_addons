...
class StockProductionLotAuditLog(models.Model):
    _name = "stock.production.lot.audit.log"
    _description = "Stock Production Lot Audit Log"
    ...
    _sql_constraints = [
        (
            "stock_production_lot_audit_log_res_id_positive",
            "CHECK(res_id > 0)",
    ...
    def create(self, vals_list):
        if not self.env.context.get("lot_audit_log_internal_create") and not self.env.user.has_group(
            "base.group_system"
        ):
            raise UserError("Audit logs are immutable and can only be created internally.")
        return super(StockProductionLotAuditLog, self).create(vals_list)
