...
class WorkcenterAuditLog(models.Model):
    _name = "workcenter.audit.log"
    _description = "Work Center Audit Log"
    ...
    _sql_constraints = [
        (
            "workcenter_audit_log_res_id_positive",
            "CHECK(res_id > 0)",
    ...
    def create(self, vals_list):
        if not self.env.context.get("workcenter_audit_log_internal_create") and not self.env.user.has_group(
            "base.group_system"
        ):
            raise UserError("Audit logs are immutable and can only be created internally.")
        return super(WorkcenterAuditLog, self).create(vals_list)
...
class ResUsers(models.Model):
    ...
    def _workcenter_audit_log_allowed_ids(self):
        self.ensure_one()
        logs = self.env["workcenter.audit.log"].sudo().search([])
        ...
        for log in logs:
            ...
            if cache_key not in access_cache:
                access_cache[cache_key] = self._workcenter_audit_log_can_read_target(model_name, log.res_id)
    ...
    def _workcenter_audit_log_can_read_target(self, model_name, res_id):
        self.ensure_one()
        try:
