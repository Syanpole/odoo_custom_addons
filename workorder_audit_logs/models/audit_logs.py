...
class MrpWorkorderAuditLog(models.Model):
    _name = "mrp.workorder.audit.log"
    _description = "MRP Work Order Audit Log"
    ...
    _sql_constraints = [
        (
            "mrp_workorder_audit_log_res_id_positive",
            "CHECK(res_id > 0)",
    ...
    def create(self, vals_list):
        if not self.env.context.get("workorder_audit_log_internal_create") and not self.env.user.has_group(
            "base.group_system"
        ):
            raise UserError("Audit logs are immutable and can only be created internally.")
        return super(MrpWorkorderAuditLog, self).create(vals_list)
...
class ResUsers(models.Model):
    ...
    def _workorder_audit_log_allowed_ids(self):
        self.ensure_one()
        logs = self.env["mrp.workorder.audit.log"].sudo().search([])
        ...
        for log in logs:
            ...
            if cache_key not in access_cache:
                access_cache[cache_key] = self._workorder_audit_log_can_read_target(model_name, log.res_id)
    ...
    def _workorder_audit_log_can_read_target(self, model_name, res_id):
        self.ensure_one()
        try:
