...
class RoutingLog(models.Model):
    _name = "routing.log"
    _description = "Routing Audit Log"
    ...
    def create(self, vals_list):
        ...
        if not self.env.context.get("routing_log_internal_create") and not self.env.user.has_group(
            "base.group_system"
        ):
            raise UserError("Audit logs are immutable and can only be created internally.")
        return super(RoutingLog, self).create(vals_list)
...
class ResUsers(models.Model):
    ...
    def _routing_log_allowed_ids(self):
        self.ensure_one()
        logs = self.env["routing.log"].sudo().search([])
        ...
        for log in logs:
            ...
            if cache_key not in access_cache:
                access_cache[cache_key] = self._routing_log_can_read_target(model_name, log.res_id)
    ...
    def _routing_log_can_read_target(self, model_name, res_id):
        self.ensure_one()
        try:
