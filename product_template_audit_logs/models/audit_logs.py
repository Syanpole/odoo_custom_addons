...
class ProductTemplateAuditLog(models.Model):
    _name = "product.template.audit.log"
    _description = "Product Template Audit Log"
    ...
    _sql_constraints = [
        (
            "product_template_audit_log_res_id_positive",
            "CHECK(res_id > 0)",
    ...
    def create(self, vals_list):
        if not self.env.context.get("product_template_audit_log_internal_create") and not self.env.user.has_group(
            "base.group_system"
        ):
            raise UserError("Audit logs are immutable and can only be created internally.")
        return super(ProductTemplateAuditLog, self).create(vals_list)
